...
public class Model {
	...
	public int simula(Country partenza) {
		Simulatore s = new Simulatore(graph);
		s.initialize(partenza, 1000);
		s.run();
		personeStanziali=s.getPersone();
		return s.getnPassi();
	}
	...
	public List<CountryAndNumber> getPersoneStanziali(){
		...
		for(Country c : personeStanziali.keySet()) {
			if(personeStanziali.get(c)!=0) {
				lista.add(new CountryAndNumber(c,personeStanziali.get(c)));
			}
		}
